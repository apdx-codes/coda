...
import { copyToClipboard, downloadFile as download } from '../utils/clipboard';
...
export function CodePreview({ result }: CodePreviewProps) {
  ...
  const downloadFile = (file: GeneratedFile) => {
    download(file.content, file.path.split('/').pop() || 'file.txt', 'text/plain');
  };
  ...
  const handleCopy = async () => {
    const success = await copyToClipboard(result.files[selectedFile].content);
    if (success) {
      setCopied(true);
